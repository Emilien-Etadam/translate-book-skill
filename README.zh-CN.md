...
输入文件 (PDF/DOCX/EPUB)
  ...
  ▼
prepare.py（确定性预处理）
  │  转换路由（PDF 简单/复杂、marker/calibre、SVG 保留）
...
- **可选（推荐用于 PDF）: Poppler** — 提供 `pdfinfo`、`pdftotext` 以启用 PDF 复杂度检测
- **可选（推荐用于复杂 PDF）: marker-pdf** — 提供 `marker_single` 进行结构化提取
...
- 转换路由（`--pdf-engine auto|calibre|marker`）与 PDF 简单/复杂判定
...
- **PDF + auto**：先用轻量启发式检测（Poppler 的 `pdfinfo` + `pdftotext -layout` 前 5 页）判断结构复杂度：
  - 大比例深缩进行（多栏版面代理特征）；
  - 页尾区域重复出现短编号行（脚注代理特征）；
  - 跨页重复的页眉/页脚行。
- 无指标则判定为 **simple**，走 Calibre。
- 有指标则判定为 **complex**，优先走 `marker_single`。
- 若检测为 complex 但未安装 `marker_single`，会给出明确 warning，并回退到 Calibre。
- SVG 处理：
...
| 复杂 PDF 转换质量差 | 使用 `--pdf-engine marker`（或安装 `marker-pdf` 后继续用 `--pdf-engine auto`）启用结构化提取 |
| 出现 `PDF complexe détecté mais marker n'est pas installé` 警告 | 安装 `marker-pdf` 让 `marker_single` 可用；若可接受质量下降，可改为强制 `--pdf-engine calibre` |
